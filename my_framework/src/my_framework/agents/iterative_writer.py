...
class IterativeWriterAgent(Runnable):
    ...
    def __init__(self, llm: BaseChatModel, max_iterations: int = 5, score_threshold: float = 0.80):
        self.llm = llm
    ...
    def invoke(self, input: dict, config=None) -> Dict:
        ...
        """
        print("\n" + "="*70)
        print("ITERATIVE WRITER AGENT - WRITE → SCORE → REFINE LOOP")
        print("="*70)
        ...
        for iteration in range(1, self.max_iterations + 1):
            ...
            print(f"   Article length: {len(current_article)} characters")
            ...
            if passes:
                print("\n" + "✅"*35)
                print(f"SUCCESS! Article accepted after {iteration} iteration(s)")
                print("✅"*35)
                return {
        ...
        print(f"Best score achieved: {max(h['score'] for h in history):.3f}")
        print("⚠️"*35)
